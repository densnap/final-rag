...
def vector_store_similarity_search(query_embedding, top_k=10, metadata_filter=None):
...
def extract_metadata_with_llm(user_query):
    ...
    if response.status_code == 200:
        ...
        print("DEBUG: Raw LLM metadata output:", content)  # Add this line
        try:
            metadata = json.loads(content)
            if isinstance(metadata, dict):
                return metadata
        except Exception as e:
            print("DEBUG: Metadata JSON decode error:", e)  # Add this line
            return None
    return None
...
def main():
    while True:
        ...
        try:
            ...
            # RAG pipeline from vector_store
            preprocessed_query = preprocess_query(user_query)
            query_embedding = get_embedding(preprocessed_query)
            metadata_filter = extract_metadata_with_llm(user_query)
            print("DEBUG: metadata_filter =", metadata_filter)  # <--- Add this line
            vector_rows = vector_store_similarity_search(query_embedding, top_k=10, metadata_filter=metadata_filter)
            ...
            # Feed both SQL and RAG context to LLM for final answer
            ...
            print("DEBUG Exception:", e)  # <--- Add this line
